...
contract Attacker is Test {
    ...
    function setUp() public {
        ...
        vm.createSelectFork(url, 15837893);
        vm.label(weth, "WETH");
        vm.label(usdc, "USDC");
        vm.label(dai, "DAI");
        vm.label(caw, "CAW");
        vm.label(tsuka, "TSUKA");
        vm.label(FEG_WETH_UniV2Pair, "FEG/WETH Pair");
        vm.label(USDC_CAW_UniV2Pair, "USDC/CAW Pair");
        vm.label(USDC_TSUKA_UniV2Pair, "USDC/TSUKA Pair");
        vm.label(KNDX_WETH_UniV2Pair, "KNDX/WETH Pair");
        preWorks();
        vm.deal(address(this), 0); // set this balance is 0 to show effect
